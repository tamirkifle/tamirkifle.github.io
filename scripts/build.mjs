...
import { stackArt, projectArt } from "./art.mjs";
...
function header(active) {
  return `<a class="skip-link" href="#main">Skip to content</a><header class="site-header wrap">
    <a class="wordmark" href="/" aria-label="Tamir Yirga, home">tamir.info</a>
    <nav class="main-nav" aria-label="Main navigation"><a href="/work.html" ${active === "projects" ? 'aria-current="page"' : ""}>Projects</a><a href="/writing.html" ${active === "writing" ? 'aria-current="page"' : ""}>Writing</a><a href="/about.html" ${active === "about" ? 'aria-current="page"' : ""}>About</a></nav>
...
<meta property="og:type" content="website"><meta property="og:title" content="${escape(title)}"><meta property="og:description" content="${escape(description)}"><meta property="og:url" content="https://tamir.info${path}"><meta property="og:image" content="https://tamir.info/media/social-card.png"><meta property="og:image:alt" content="Tamir Yirga — Projects and engineering notes"><meta name="twitter:card" content="summary_large_image">
...
function projectRow(project) {
  ...
  return `<article class="project-row"><div class="project-copy"><h3><a href="${projectURL(project)}">${escape(project.name)}<span class="project-subtitle">— ${escape(project.title)}</span></a></h3><p class="project-description">${escape(project.description)}</p><div class="project-reading"><a class="reading-label" href="${projectURL(project)}#notes">Notes (${notes.length})</a><ul>${notes
    ...
    .join(
      "",
    )}</ul>${notes.length > 3 ? `<a class="all-notes" href="${projectURL(project)}#notes">All notes</a>` : ""}</div></div>${project.slug === "inferrs" ? inferenceDiagram() : `<a class="project-visual visual-${project.art}" href="${projectURL(project)}" aria-label="Read about ${escape(project.name)}">${projectArt(project.art)}</a>`}</article>`;
}
function archiveRows(projects) {
  return `<div class="archive-list">${projects.map((project) => `<a class="archive-row" href="${projectURL(project)}"><div><h3>${escape(project.name)}</h3><p>${escape(project.title)}</p></div><span aria-hidden="true">→</span></a>`).join("")}</div>`;
}
...
await emit("index.html", {
  title: "Tamir Yirga — Projects & Notes",
  description:
    "Projects and engineering notes on inference engines, distributed systems, and the code behind them.",
  ...
  <section class="wrap projects-section" id="projects"><h2 class="section-title">Projects</h2>${selected.map(projectRow).join("")}<div class="other-projects"><h2 class="section-title">Other projects</h2>${archiveRows(site.projects.filter((project) => !project.selected))}</div></section>`,
});
await emit("work.html", {
  title: "Projects — Tamir Yirga",
  description:
    "Inference, distributed storage, data pipelines, and computer vision. Project overviews and related writing.",
  active: "projects",
  body: `<header class="page-intro wrap"><h1>Projects</h1><p>What I’m building, how it works, and notes along the way.</p></header><section class="wrap projects-section" aria-label="Projects">${selected.map(projectRow).join("")}<div class="other-projects"><h2 class="section-title">Other projects</h2>${archiveRows(site.projects.filter((project) => !project.selected))}</div></section>`,
});
